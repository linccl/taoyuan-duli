...
<script setup lang="ts">
  ...
  const LINUX_DO_ERROR_MESSAGES: Record<string, string> = {
    ...
    profile: '获取 Linux DO 用户资料失败，请重试', userinfo_failed: '获取 Linux DO 用户资料失败，请重试', subject_mismatch: 'Linux DO 账号信息不一致，请重新登录', oauth_store: '保存 Linux DO 登录状态失败，请稍后重试', id_token_invalid: 'Linux DO 身份令牌校验失败，请重试',
    ...
    local_banned: '当前账号暂不可登录', local_deleted: '当前账号暂不可登录', auto_create: '自动创建账号失败，请稍后重试', account_create: '自动创建账号失败，请稍后重试', auto_create_disabled: '当前暂未开放 Linux DO 自动创建账号', username_conflict: '自动创建账号失败，请稍后重试'
  }
